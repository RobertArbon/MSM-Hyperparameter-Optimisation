...
\begin{enumerate}
    ...
    \item Chignolin 2 - best overall from Bayesian optimisation. Hpix = 218/NA
    \item Chignolin 3 - best logit(dist) from random sampling (and overall). Hpix = 23/24
    \item BBA 1 - best overall from random sampling. Shows misfolding as t2 (folded -> misfolded). Hpix = 24/24
    \item BBA 2 - best dihedrals from random sampling. Hpix = 6/6
    \item BBA 3 - best distances from random sampling. Hpix = 58/85
    \item BBA 4 - best from Bayesian optimisaiton (of just the logit(dist)?). 227/NA
    \item BBA 5 - second best overall.  124/262
\end{enumerate}
...
\begin{table}
    ...
    \midrule
     ...
     BBA          & 2 & 41 & \makecell[tl]{logit(dist.) \\(8.0, 2.9)} & 289 & 67 & 18 & Random & NA & 140 & $t_{2}$ & \makecell[tc]{9.7 \\ (2.1, 188.7)}  \\
     ...
     BBA          & 5 & 41 & \makecell[tl]{logit(dist.,)\\(7.7, 0.33)} & 444 & 53 & 20 & Bayesian & 136 & 100 & $t_{2}$ &\makecell[tc]{42.8 \\ (4.7, 286.3)}  \\
    \bottomrule
    \end{tabularx}
...
This model has the largest median $t_{2}$ using the logistic distance feature.

\begin{figure}[h]
    \centering
    \includegraphics[width=0.5\columnwidth]{missing.pdf}
