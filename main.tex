...
\affiliation{ReDesign Science, 180 Varick St, New York, NY 10014, USA}
\author{Yanchen Zhu}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\title[]{Markov state model hyperparameter optimization}
...
The uncertainty for model observables was estimated using the bootstrap with \num{100} bootstrap samples. The point estimate and error bars were calculated as the median,   \SI{2.5}{\percent} \& \SI{97.5}{\percent} quantiles of the distribution from the bootstrap samples.
...
Having created the hyperparameter trial data set we first investigate some inconsistencies in the VAMP2 scores; then we make some observations regarding optimization using random selection and Bayesian optimization; and finally we determine what effect the lag time and number of scored eigenvectors makes on model selection.  
