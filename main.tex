...
\begin{enumerate}
    \item Theory: sensitivity analysis. Need to reference paper and precise the metrics used. 
    \item Table 1 - finish. 

    \item Methods: Sensitivity analysis. 

    \item Results: first section should be: 'Eigenvectors change definition during optimisation' . Re-write with CLN and BBA. 
    \item Change conclusions. 
\end{enumerate}
  Abstract
\end{abstract}
...
%     \item Can optimisation be a useful tool to optimise hyperparameters of an MSM? 
...
% We build on previous work in this area~\cite{Optimized_2016} and use as our motivating examples the fast folding proteins BBA and Chignolin from the benchmark data set provided by D. E. Sharw~\cite{lindorff-larsen_how_2011}. 
...
The methods and intended logic of this paper can be summarised as follows: 
\begin{enumerate}
    \item We looked at two standard protein folding benhcmark systems: Chignolin and BBA, using the same data as in many other previous studies.  
    \item We created a random set of hyperparameters which covered a range of values also used in many previous similar studies [refs]. 
    \item For each set of hyperparameters (a \emph{trial}) we fit an MSM and estimated bootstrapped values the eigenvalues, implied timescales and VAMP scores for a range of different MSM lag-times and scored eigenvectors. This consituted our \emph{hyperparameter trial dataset}.  
    \item using this dataset we can test whether Bayesian optimisation can produce 'better' models. 
    \item in addition we hoped to use statistical regresion to quantify how the each hyperparameter affects the model quality. 
\end{enumerate}
...
\begin{itemize}
    \item Can you put the details about what and how you optimised (include all the various permutations in a table).  
...
\begin{table}
    \caption{\textbf{Hyperparameter optimisation tasks}}
    \begin{tabularx}{\textwidth}{llXXXXX}
    \toprule
    \textbf{Protein} & \textbf{Objective} & \textbf{Feature} & \textbf{Initial Data} & \textbf{No. Trials} & \textbf{Gamma} \\ 
    
    \midrule
    Chignolin & t2        & All & 136 & 95 & 25 \\
    Chignolin & t2, t2/t3 & All & 55 & 141 & 25 \\
    Chignolin & VAMPeq(2) & All & 136 & 100 & 25 \\
%%    Chignolin & VAMPeq(2), t2/t3 & All & 55 & 150 & 25 \\
    Chignolin & VAMPeq(2), VAMPeq(2)/VAMPeq(3) & All & 55 & 150 & 25 \\

    BBA & t2        & All & 136 & 100 & 25 \\
    BBA & t2, t2/t3 & All & 136 & 100 & 25 \\
    BBA & VAMPeq(2) & All & 136 & 100 & 25 \\
    BBA & VAMPeq(2), VAMPeq(2)/VAMPeq(3) & All & 136 & 100 & 25 \\
    \bottomrule
    \end{tabularx}
    \label{tab:data_description}
\end{table}
...
\subsection{Eigenvectors definitions change with hyperparameters}

\textbf{Figure 1}
\begin{itemize}
    \item Each panel shows timescales ranked for BBA and Chignolin 
    \item Two models are highlighted with arrows for BBA and Chignolin.  
    \item BBA model 1 - highest t2; model 2 - second highest t2 (same feature)
    \item Cln25 model 1 - highest t2; model 2 - highest t2 with different feature. 
\end{itemize}


\textbf{Figure 2}
Shows difference between BBA model 1 and model 2 with pymol structures for dominant eignevector process, overlaid on FES in tica space. Put similar Cln25 in SI. 


Given our hyperparameter trial dataset our first task was to inspect the models to validate the models beyond the VAMP scores .  For reasons that we shall explain in [VAMP section] we ranked the hyperparameter trials by the median value of $t_2$ rather thanthe VAMP score, which we expected (from previous literature, Schere 2018) to be correlated with the slow, protein folding process.  

Figure 1 shows $t_2$ for chignolin (panel a) and BBA (panel b) on the vertical axis, running left to right, highest value of $t_2$ on the left. As you can see, the range of values of Chignolin are small, with a few trials with small values of $t_2$.  The dominant eigenvector of the model with the largest value of $t_2$ descibes two metastable states (figure A1), folded and unfolded. The narrow range of values and relative insensitivity of $t_2$ with chaning hyepraparmeter values suggest this model is highly robust. 

The situation for BBA is different.  Here we see a large (2-3X) difference in $t_2$ across the whole trial dataset,  with the 'best'  trial (model 1) with a $t_2$ twice as large as the next trial (model 2). Inspection of the two eigenvectors reveal that they describe two different processes.  In model 2, one extreme of the dominant eigenvector corresponds to the folded protein, while in model 1, the extremes of the dominant  eigenvector show two distinct mis-folded states.  

If the two dominant eigenvectors in models 1 and 2 are distinct then one cannot be said to be 'better' or more optimised that the other as they correspond to two different processes.  this then reveals the first problem with optimising a single metric based on the eigenvectors of the transition matrix: the \emph{definition of the eigenvectors can change with hyperaparameters, not just accuracy.}
...
\subsection{Bayesian optimization does not improve timescales}

Figure 5
\textbf{Ryan}
\begin{itemize}
    \item One column for BBA, one for CLN. 
    \item One panel for t2 and t2 + t2/t3; one panel for VAMPeq2, and VAMPeq2 + VAMPeq2/VAMPeq3 (so axes are sensible) 
    \item add Random results in one shape and colour. t2 results in another shape and colour. t2 + t2/t3 in another shape in colour (i.e., three different optimisations).
    \item Do same again but for VAMPeq optimisation.
\end{itemize}
