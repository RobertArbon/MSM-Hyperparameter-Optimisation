...
\begin{figure}[ht]
    \begin{minipage}[b]{0.49\linewidth}
        \centering
        \subcaption{k=2\label{fig:vamp_vs_timescales_k2}}{\includegraphics[width=\linewidth]{figures/1fme_timescale_comarison_by_rank_k_2.png}}
    \end{minipage}
    \begin{minipage}[b]{0.49\linewidth}
        \centering
        \subcaption{k=10\label{fig:vamp_vs_timescales_k10}}{\includegraphics[width=\linewidth]{figures/1fme_timescale_comarison_by_rank_k_10.png}}
    \end{minipage}
    ...
    % \begin{minipage}[b]{0.49\linewidth}
    %     \centering
    %     \subcaption[]{5LRM\label{fig:5lrm_whole}}{\includegraphics[width=\linewidth]{chapters/chapter01/structures/5lrm_whole.pdf}}
    % \end{minipage}
    % \begin{minipage}[b]{0.49\linewidth}
    %     \centering
    %     \subcaption[]{5LRM active site\label{fig:5lrm_AS}}{\includegraphics[width=\linewidth]{chapters/chapter01/structures/5lrm_AS.pdf}}
    % \end{minipage}
...
% The high relevance of the $m$ is not surprising in light of the bias-variance trade-off~\cite{friedman2001elements}: with an insufficient number of TICA dimensions, the resulting discrete basis (from the clustering step), will not adequately describe the true eigenvectors of the propagator. Too many TICA dimensions may result in dimensions which only capture noisy fluctuations in the data, resulting in poor convergence to suitable cluster definitions. However,  given the small values of $m$ sampled, this regime is unlikely to have been reached. 

% The low relevance of $n$ is perhaps more surprising. It would be expected that both $n$ and $m$ would behave in similar ways, creating highly biased discrete states for low values of $n$ and $m$, and highly variable cluster definitions for large $m$ and $n$ (the bias-variance trade-off for $n$ in MSM construction is well documented~\cite{mcgibbonVariationalCrossvalidationSlow2015, wuVariationalApproachLearning2020c}). It is possible that we do not see an effect on the VAMP score for large values of $n$ as range of values sampled here ($10 < n <500$) is comparable to the $\sqrt{N}$ heuristic~\cite{Ward_Clust2017} for the number of cluster centers ($\sqrt{N} \simeq 180$ for the case of BBA). This does not explain why for very low values of $n$ we do not see a decrease in the VAMP score. 
...
% Center (\si{\angstrom})        &                  - &               12.7 &                   - &                     -  \\
% Steepness (\si{\per\angstrom}) &                  - &                1.2 &                   - &                     -  \\
% TICA lag (ns)                  &                 75 &                 36 &                  72 &                     3  \\
% TICA dimension                 &                 14 &                  7 &                   4 &                    11  \\
% Num. clusters                  &                 23 &                435 &                 475 &                   404  \\
% Num. Dominant                  &                  2 &                  2 &                   2 &                     2  \\
% Loss(k=2)                      &  1.5 \%, [1.9-0.7] &  1.6 \%, [1.9-1.3] &   1.8 \%, [2.1-1.5] &    9.4 \%, [14.9-5.4]  \\
% Loss(k=3)                      &  2.1 \%, [2.7-1.5] &  2.6 \%, [3.8-2.0] &   4.1 \%, [7.3-2.4] &  15.5 \%, [21.7-11.4]  \\
% Loss(k=4)                      &  2.6 \%, [3.4-2.0] &  3.7 \%, [5.3-2.6] &  7.1 \%, [10.7-4.3] &  20.2 \%, [26.6-15.9]  \\
% \bottomrule
% \end{tabular}
...
% The extended model selection procedure also suffers from the problem that there is no theoretical guide to the optimal trade-off between the timescale gap and the quality of the dominant eigenvectors. The truncation error (VAMPE) score does treat the number of eigenvectors as another hyperparameter~\cite{wuVariationalApproachLearning2020c} but this does not fully resolve the problem as is treats the error arising due to a poor set of hyperparameters on a equal footing to the truncation error. i.e., it does not differentiate between many eigenvectors with poor accuracy and a smaller number of very accurate eigenvectors. 


% These problems with model selection highlight the fact that there will likely be not automatic model selection procedure for MSM primarily based on VAMP scores. The VAMP scores guide hyperparameter selection but model observables should be carefully inspected and alternative hyperparameters should still be considered. 
...
% %     \item the relevance of each hyperparameter is the inverse of the characteristic length scale of ind≥
