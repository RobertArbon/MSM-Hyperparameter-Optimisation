...
% The VAMP formalism provides an overarching framework for variationally optimising basis states for both reversible and non-reversible (and non-stationary) MSMs. For reversible MSM any given trial set of basis states will give rise to transition matrix where the sum of powers of the eigenvalues  will be bounded from above by the true sum. In order to calculate these scores the following formula is used (eq. 24 in []): 

% \begin{equation}
% \max _{\mathbf{U}, \mathbf{V}} \operatorname{VAMP2}=\left\|\mathbf{C}_{00}(\mathbf{w})^{-\frac{1}{2}} \mathbf{C}_{01}(\mathbf{w}) \mathbf{C}_{11}(\mathbf{w})^{-\frac{1}{2}}\right\|_{r}^{r},
% \end{equation}\label{eqn:vamp_def}

% [explanation].  For reversible processes $\mathbf{C}_{00} = \mathbf{C}_{tt}$ (and $\mathbf{U} = \mathbf{V}$) however due to finite amount of data the relation will not be true in practice. As we show in the appendix, small discrepancies in these count matrices can lead to the following paradoxical results: 
% \begin{itemize}
%     \item Two models may have the same VAMP2 score and have very different timescale spectra. 
%     \item Two models may have VAMP2 scores anti-correlated with their timescales. 
% \end{itemize}

% This is shown for the hyperparameter trials in figure~\ref{fig:bad_vamp_scores}. In panels (a) and (c) the  model with the highest VAMP2(k=2) score is compared to two other models which have lower median VAMP2 scores yet slower timescales.  The fourth ranked model is model with the highest implied timescale.  Panels (b) and (d) show this effect for models compared to the seventh ranked model.  
